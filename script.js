(function(){
  ...
  function actualizarSugerenciasVitales(){
    ...
    if(fc!==null || tas!==null){
      ...
      sugerC.textContent = txt ? txt+' — según signos vitales, confirmá con el examen' : '';
    } else { sugerC.textContent=''; }
    if(fr!==null){
      let txt='';
      if(fr>=10 && fr<=36){ txt='Sugerido: 2 pts (FR en rango normal)'; }
      else { txt='Sugerido: 1 pt (FR fuera de 10-36)'; }
      sugerR.textContent = txt+' — según signos vitales, confirmá con el examen';
  ...
  function generarResumen(){
    ...
    let out = '';
    out += `HISTORIA CLÍNICA — FORMATO MIST\n`;
    ...
    out += `\nI — HALLAZGOS (antecedentes, examen, diagnóstico)\n`;
    out += `Antecedentes: ${v('f_antecedentes') || '-'}\n`;
    out += `Medicación habitual: ${v('f_medicacion') || '-'}\n`;
    out += `Alergias: ${v('f_alergias') || '-'}\n`;
    out += `Examen físico: ${v('f_examen') || '-'}\n`;
    out += `Diagnóstico presuntivo: ${v('f_diagnostico') || '-'}\n`;

    // S — Signs (vitales + scores)
    out += `\nS — SIGNOS (vitales y scores)\n`;
    ...
    if(cTotalTxt !== '—'){
      out += `CRAMP: ${cTotalTxt}  (${document.getElementById('cramp_badge').textContent})\n`;
    ...
    if(scqTotal !== '0%' || quemNotas){
      out += `Quemados — SCT (${scqMode==='adult'?'adulto':'pediátrico'}): ${scqTotal}  (${document.getElementById('scq_badge').textContent})\n`;
  ...
  document.getElementById('btnLimpiar').addEventListener('click', ()=>{
    ...
    ['f_nombre','f_edad','f_fecha','f_hora','f_motivo','f_antecedentes','f_medicacion','f_alergias','f_examen','f_diagnostico','f_plan','v_fc','v_fr','v_temp','v_tas','v_tad','v_sat'].forEach(id=>{
